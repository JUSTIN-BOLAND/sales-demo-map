...
var pulsatingMAH = true;
var pulsatingINPEX = true;
...
function addPoints(data) {
    ...
    function drawPoints(){                                                        //function to draw points. Called whenever filters are applied or heatmap or clusters are enabled and disabled.
		...
		*/
		...
		/**
		 * Filters the points within INPEX with names selected (INPEX Account Owners)
		 */
		...
		for(var row = 0; row < filtered.length; row++) {
			if (filtered[row].lat > 0){
				...
				marker.bindPopup("<dl>"
					+ "<h2>Account Information</h2><dt>Account Name </dt><dd>" + filtered[row].client 
					+ "</dd><dt>Address </dt><dd>"+ filtered[row].address
					+  "</dd><dt>2018 Sales </dt><dd>$" + numeral(filtered[row].sales_2018).format('0,0') 
					+ "</dd><dt>Sales Segment </dt><dd>" +tier_no+ "</dd><dt>Growth Potential </dt><dd>" + filtered[row].sales_potential 
					+"</dd><dt>H1 2018</dt><dd>$"+numeral(filtered[row].h1_2018).format('0,0')
					+"</dd><dt>H1 2019</dt><dd>$"+numeral(filtered[row].h1_2019).format('0,0')
					+"</dd><dt>Sales Trend </dt><dd>" + trend.toFixed(2) 
					+"%</dd><dt>Trend Category</dt><dd>" + filtered[row].trend
					+"</dd><dt>Account Owner</dt><dd>"+filtered[row].account_owner
					+ "</dd><dt>INPEx AO</dt><dd>"+filtered[row].inpex_ao
					+ "</dd></dl>", 
				{
				maxWidth: 390,
				maxHeight: 700
				...
				*/
				...
				var pulsationINPEX;
					/*
					** pulsationMAH stores the animation for the inpex circles. If pulsatingMAH is false, then pulsationMAH will be an empty string (no animation)
					*/
				if (pulsatingMAH === true){
					pulsationMAH = '<animate attributeName="r" from="49" to="63" dur="1.8s" begin="0s" repeatCount="indefinite"/><animate attributeName="opacity" from="1" to="0" dur="1.8s" begin="0s" repeatCount="indefinite"/>';
					//pulsating = !pulsating;
				}
				else{
					pulsationMAH = '';
					...
					/*
					** pulsationINPEX stores the animation for the inpex circles. If pulsatingINPEX is false, then pulsationINPEX will be an empty string (no animation)
					*/
				if (pulsatingINPEX === true){
					pulsationINPEX = '<animate name="animateopacityINPEX" attributeName="opacity" from="1" to="0" dur="2s" begin="0s" repeatCount="indefinite"/>';
				}
				else{
					pulsationINPEX = '';
				}
				if (filtered[row].inpex_mah === "MAH"){
					pulse = '<circle cx="64" cy="67" fill="none" r="52" stroke="blue" stroke-width="8">'+pulsationMAH+'</circle>';
					
				}

				/*
				**if the marker is for an INPEX client, the outer circle will be coloured with a color corresponding to each account owner
				*/
				else{
					pulse = '<circle cx="64" cy="67" fill="none" '+pulsationINPEX+'</circle>'
					var clientColor;
					...
					pulse = '<circle cx="64" cy="67" fill="none" r="52" stroke="'+clientColor+'" stroke-width="8">'+pulsationINPEX+'</circle>';
					}
			...
			//Loop to store client data inside drawn polygon	
				...
				.setContent("<dl class='summary'><h2>INPEx/MAH</h2><dt>No. of INPEx accounts</dt>"+inpex
				+"<dt>No. of MAH accounts</dt><dd>"+mah
				+"</dd><h2>Sales Segment</h2><dt>No. of Segment 1 accounts</dt><dd>"+totTier1
				+"</dd><dt>No. of Segment 2 accounts</dt><dd>"+totTier2
				+"</dd><dt>No. of Segment 3 accounts</dt><dd>"+totTier3
				+"</dd><dt>No. of Segment 4 accounts</dt><dd>"+totTier4
				+"</dd><dt>No. of accounts with no segment</dt><dd>"+noTier
				+"</dd><h2>Growth Potential</h2><dt>Low</dt><dd>"+totLow
				+"</dd><dt>Moderate</dt><dd>"+totMod
				+"</dd><dt>High</dt><dd>"+totHigh
				+"</dd><dt>Without growth info</dt><dd>"+noPotential
				+"</dd><h2>Sales</h2><dt>Total sales 2017</dt><dd>$"+numeral(sales2017).format('0,0')
				+"</dd><dt>Total sales 2018</dt><dd>$"+numeral(sales2018).format('0,0')
				+"</dd><dt>Average sales 2017</dt><dd>$"+numeral(avg2017.toFixed(2)).format('0,0')
				+"</dd><dt>Average sales 2018</dt><dd>$"+numeral(avg2018.toFixed(2)).format('0,0')
				+"</dd><dt>H1'2018</dt><dd>$"+numeral(toth12018).format('0,0')
				+"</dd><dt>H1'2019</dt><dd>$"+numeral(toth12019).format('0,0')
				+"</dd><dt>H1 Trend</dt><dd>"+h1trend.toFixed(2)+"%"
				+"</dd></dl><div style='text-align: center;'><button id='download'>Download selected</button></div>")
				.addTo(map);
			    dialog.hideResize();
			...
			*/
			...
			$('input[name="pulsationMAH"]').click(function(e){
				pulsatingMAH = !pulsatingMAH;
				drawPoints();
				if(pulsatingMAH===false){
					var circleLegend = document.getElementById("animateradiusMAH");
					circleLegend.setAttribute("from", "30");
					circleLegend.setAttribute("to", "30");
					var circleOpacit = document.getElementById("animateopacityMAH");
					...
					var circleOpacity = document.getElementById("animateopacityMAH");
					circleOpacity.setAttribute("to", "0");
				}
				
				});
				$('input[name="pulsationINPEX"]').click(function(e){
					pulsatingINPEX = !pulsatingINPEX;
					drawPoints();
					if(pulsatingINPEX===false){
						var circleOpacity = document.getElementsByName("animateopacityINPEX");
						for(var i = 0; i < circleOpacity.length; i++) {
							circleOpacity[i].setAttribute("to", "1");
						}
					}
					else{
						var circleOpacity = document.getElementsByName("animateopacityINPEX");
						for(var i = 0; i< circleOpacity.length; i++) {
							circleOpacity[i].setAttribute("to", "0");
						}
					}
